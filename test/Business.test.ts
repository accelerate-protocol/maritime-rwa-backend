...
describe("RWA:", function () {
  ...
  it("tc-47", async function () {
    ...
    // Ensure minimum deposit amount is met
    // Convert minDepositAmount from contract to the correct scale for distribution
    // The minDepositAmount in the contract is "10000000" which is 10 * 10^6 in wei
    // When divided by 1e18, it becomes 0.01, but we need at least 10 for each investor
    const adjustedMinAmount = Math.max(minAmount, 10); // Ensure minimum is at least 10
    const distribution = distributeMoneyWithMinimum(
      totalSupply,
    ...
    // Ensure minimum dividend amount is reasonable
    const dividendCountArr = distributeMoneyWithMinimum(
      principalInterest,
      dividendCount,
      10 // Use a higher minimum value to ensure meaningful dividends
    );
    const totalNav = await USDT.balanceOf(depositTreasury) - depositTreasuryBalance;
    console.log("总派息资产:", totalNav.toString())
  ...
  it("tc-49", async function () {
    ...
    const dividendAmount = BigInt(Math.floor(1000)) * decimalFactor;
    await expect(USDTdepositTreasury.transfer(rbfDividendTreasury, dividendAmount)).not.to.be.reverted;
  ...
  it("tc-50", async function () {
    ...
    for (let i = 0; i < whitelistLength; i++) {
      ...
      const manageFee = await vaultInvest.manageFee();
      const investAmount = BigInt(Math.floor(distribution[i])) * await getDecimalFactor(VAULT);
    ...
    expect(price).to.equal(nav * await getDecimalFactor(VAULT) / Supply);
    ...
    const principalInterest = Math.floor(totalSupply * randomMultiplier);
    //  const waitMint = BigInt(Math.floor(principalInterest - totalSupply) * 1e6);
    const waitMint = BigInt(Math.floor(principalInterest - totalSupply)) * await getDecimalFactor(VAULT);
    ...
    for (let i = 0; i < dividendCountArr.length; i++) {
      ...
      const dividendAmount = BigInt(Math.floor(dividendCountArr[i])) * await getDecimalFactor(VAULT);
    ...
    expect(price_final).to.equal(nav_final * await getDecimalFactor(VAULT) / Supply_final);
  });
  ...
  it("tc-51", async function () {
    ...
    var vaultDividendBalance: any;
    for (let i = 0; i < dividendCountArr.length; i++) {
      // Ensure dividend amount is positive
      const dividendValue = Math.max(0, dividendCountArr[i]);
      const dividendAmount = BigInt(Math.floor(dividendValue)) * await getDecimalFactor(VAULT);
      console.log("第" + (i + 1) + "次派息:", dividendAmount);
      await expect(USDTdepositTreasury.transfer(rbfDividendTreasury, dividendAmount)).not.to.be.reverted;
      await expect(rbfManager.dividend()).not.to.be.reverted;
