...
describe("V2 架构完整业务流程测试", function () {
    ...
    beforeEach(async function () {
        ...
        creation = await CreationFactory.deploy(
    ...
    describe("公开发售", function () {
        ...
        describe("1. 基础设置测试", function () {
            ...
            it("应该正确设置收益模块初始状态", async function () {
                const globalPool = await accumulatedYield.getGlobalPoolInfo();
                expect(globalPool.isActive).to.be.true; // 初始化时应该为true
        ...
        describe("8. offChainRedeem 操作测试", function () {
            ...
            it("应该拒绝在融资成功后执行offChainRedeem", async function () {
                // 先进行deposit
        ...
        describe("9. 纯链下认购 Dividend 操作测试", function () {
        ...
        describe("11. 纯链下认购完成后提取费用", function () {
        ...
        describe("12. 转账相关操作", function () {
            ...
            it("应该拒绝在激活代币前转账", async function () {
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
                await fund.connect(manager).offChainDeposit(depositAmount_user1, user1.address);
            ...
            it("应该在融资完成后可以激活代币", async function () {
                expect(await token.paused()).to.be.true;
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
            ...
            it("应该拒绝重复激活代币", async function () {
                ...
                await expect(fund.connect(manager).unpauseTokenOnFundingSuccess()).to.revertedWith("Pausable: not paused");
            });

            it("应该拒绝非管理员激活代币", async function () {
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
                await fund.connect(manager).offChainDeposit(depositAmount_user1, user1.address);
                ...
                await expect(fund.connect(user1).unpauseTokenOnFundingSuccess()).to.revertedWith("Crowdsale: only manager");
            });
            it("应该拒绝在融资完成前激活代币", async function () {
                expect(await token.paused()).to.be.true;
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
    ...
    describe("白名单模式", function () {
        ...
        describe("9. 白名单模式下纯链下认购 Dividend 操作测试", function () {
            ...
            it("应拒绝非金库批准的情况下执行分红", async function () {
                // 先解锁代币交易
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
        ...
        describe("10. 白名单模式下链下认购 Claim 操作测试", function () {
            ...
            it("应该拒绝在融资成功后激活收益前提取分红", async function () {
                ...
                await usdt.connect(dividendTreasury).approve(await accumulatedYield.getAddress(), dividendAmount);
                await expect(accumulatedYield.connect(dividendTreasury).distributeDividend(dividendAmount, signature2)).to.be.revertedWith("AccumulatedYield: pool not active");
                ...
                // 执行claim失败
                await expect(accumulatedYield.connect(user1).claimReward()).to.be.revertedWith("AccumulatedYield: pool not active");
                ...
                //执行claim成功
                await expect(accumulatedYield.connect(user1).claimReward()).to.not.reverted;
        ...
        describe("11. 白名单模式下纯链下认购完成后提取费用", function () {
            ...
            it("应拒绝非feeReceiver提取融资额", async function () {
                // 先进行deposit
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
        ...
        describe("12. 白名单相关操作", function () {
            ...
            it("应该能在白名单之间进行转账", async function () {
                // 先进行deposit
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
            ...
            it("应该不能多次分红使用一个签名", async function () {
                // 先进行deposit
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
                ...
                const dividendAmount = ethers.parseUnits("10000", TEST_CONFIG.TOKEN_DECIMALS);
                var signature2 = await prepareDividendSignature(dividendAmount);
                ...
                //继续执行分红
                // signature2 = await prepareDividendSignature(dividendAmount);
                await usdt.connect(dividendTreasury).approve(await accumulatedYield.getAddress(), dividendAmount);
                await expect(
                    accumulatedYield.connect(dividendTreasury).distributeDividend(dividendAmount, signature2)
                ).to.revertedWith("AccumulatedYield: invalid signature");
    
            });
    
            it("应该不能向不在白名单中的账户进行转账", async function () {
                // 先进行deposit
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
            ...
            it("管理员应该能重新启用白名单", async function () {
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
                await fund.connect(manager).offChainDeposit(depositAmount_user1, user1.address);
                ...
                var actualSharesUser1_1 = await token.balanceOf(user1.address);
                expect(actualSharesUser1_1).to.be.equal(actualSharesUser1 - transferAmount);
                var actualSharesUser2 = await token.balanceOf(user2.address);
                expect(actualSharesUser2).to.be.equal(depositAmount_user2 * TEST_CONFIG.SHARE_PRICE_DENOMINATOR / TEST_CONFIG.SHARE_PRICE + transferAmount);
            });
    
    
            it("禁用白名单后重新启用白名单，应该不影响分红", async function () {
                // 先进行deposit
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
            ...
            //TODO
            it("应该拒绝管理员对禁用白名单重复禁用", async function () {
                var depositAmount_user1 = ethers.parseUnits("1000", TEST_CONFIG.TOKEN_DECIMALS);
                var offDepositSignature = await prepareOffDepositSignature(depositAmount_user1, user1.address);
                await usdt.connect(user1).approve(await fund.getAddress(), depositAmount_user1);
                await fund.connect(manager).offChainDeposit(depositAmount_user1, user1.address);
            ...
            //TODO
            it("应该拒绝管理员在启用白名单情况下再次启用白名单", async function () {
                await vault.connect(manager).enableWhitelist();
            });
    
            it("应该拒绝添加已在白名单的账户入白名单", async function () {
                await vault.connect(manager).addToWhitelist(user3.address);
                await expect(
                    vault.connect(manager).addToWhitelist(user3.address)
                ).to.revertedWith("BasicVault: already whitelisted");
            });
    
    
            it("应该拒绝从白名单中删除不在白名单的账户", async function () {
                await vault.connect(manager).addToWhitelist(user3.address);
                await vault.connect(manager).removeFromWhitelist(user3.address);
                await expect(
                    vault.connect(manager).removeFromWhitelist(user3.address)
                ).to.revertedWith("BasicVault: not whitelisted");
            });
