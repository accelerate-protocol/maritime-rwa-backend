...
describe("bsc testnet deploy", function () {
    // 代币精度常量
    ...
    describe("部署合约", function () {
        ...
        it("应部署合约", async function () {
            ...
            // 给测试用户分配初始余额
            await usdt.mint(user1.address, ethers.parseUnits("1000000000000", TEST_CONFIG.TOKEN_DECIMALS));
            await usdt.mint(user2.address, ethers.parseUnits("1000000000000", TEST_CONFIG.TOKEN_DECIMALS));
            await usdt.mint(user3.address, ethers.parseUnits("1000000000000", TEST_CONFIG.TOKEN_DECIMALS));
            await usdt.mint(manager.address, ethers.parseUnits("1000000000000", TEST_CONFIG.TOKEN_DECIMALS));
            await usdt.mint(dividendTreasury.address, ethers.parseUnits("1000000000000", TEST_CONFIG.TOKEN_DECIMALS));
